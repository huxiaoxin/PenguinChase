...
NSString *const kSvNull = @"null";

@interface SvUDIDTools ()
@end
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    NSString *accessGroup = [self keyChainUDIDAccessGroup];
    if (accessGroup != nil)
    {
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    [dictForAdd setValue:[NSString stringWithUTF8String:kKeychainUDIDItemIdentifier] forKey:[NSString stringWithFormat:@"%@",kSecAttrDescription]];
...
+ (NSString *)keyChainUDIDAccessGroup {
    NSString *teamGroup = [UIDevice ly_bundleSeedID];
    NSString *bundleId = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleIdentifier"];
    return [NSString stringWithFormat:@"%@.%@", teamGroup, bundleId];
}
