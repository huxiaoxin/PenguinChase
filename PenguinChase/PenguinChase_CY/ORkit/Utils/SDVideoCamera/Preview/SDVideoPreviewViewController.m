...
- (void)popViewControllerAction {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (self->_audioPlayItem != nil) {
            bgAudioAsset = self->_audioPlayItem.asset;
            bgAudioVolume = self->_audioPlayer.volume;
        }
        ...
        // 缩放倍数
        CGFloat widthScale = SDVideoSize.width/GK_SCREEN_WIDTH;
        CGFloat heightScale = SDVideoSize.height/GK_SCREEN_HEIGHT;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //
                ...
                if (self.config.returnBlock != nil) {
                    self.config.returnBlock(mergeVideoPathString,self.videoAssetArray);
                }
...
- (UIButton *)returnButton {
    ...
    if (_returnButton == nil) {
        _returnButton = [[UIButton alloc] initWithFrame:CGRectMake(15, GK_STATUSBAR_HEIGHT, 40, 40)];
...
- (UIButton *)nextButton {
    ...
    if (_nextButton == nil) {
        ...
        _nextButton.backgroundColor = LGDMianColor;
