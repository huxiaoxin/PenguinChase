...
#import "PandaMovieLineLoadingView.h"

#define GKLineLoadingDuration  0.75
#define GKLineLoadingLineColor [UIColor whiteColor]

@implementation PandaMovieLineLoadingView

+ (void)PandaMovieshowLoadingInView:(UIView *)view withLineHeight:(CGFloat)lineHeight {
    PandaMovieLineLoadingView *loadingView = [[PandaMovieLineLoadingView alloc] initWithFrame:view.frame lineHeight:lineHeight];
...
+ (void)hideLoadingInView:(UIView *)view {
    ...
    for (UIView *subView in subviewsEnum) {
        if ([subView isKindOfClass:[PandaMovieLineLoadingView class]]) {
            PandaMovieLineLoadingView *loadingView = (PandaMovieLineLoadingView *)subView;
