...
@property(nonatomic,strong) PandaMovieSendingView * pandaSendView;
@end
...
- (void)setupUI
{
    ...
    [self.view addSubview:self.pandaSendView];
...
- (PandaMovieSendingView *)pandaSendView{
    if (!_pandaSendView) {
        _pandaSendView = [[PandaMovieSendingView alloc]initWithFrame:CGRectZero];
        GNHWeakSelf;
        _pandaSendView.clickDragViewBlock = ^(WMDragView *dragView) {
            PandaMovieSendingViewController * pandaVc = [[PandaMovieSendingViewController alloc]init];
            pandaVc.hidesBottomBarWhenPushed = YES;
            [weakSelf.navigationController pushViewController:pandaVc animated:YES];
        };
    }
    return _pandaSendView;
}
-(void)viewDidLayoutSubviews{
    _pandaSendView.frame = CGRectMake(self.view.bounds.size.width-55, self.view.bounds.size.height-55, 50, 50);
}
